...
pdf("gc_def.pdf", height = 8, width = 12)
times<-seq(0,0.9, length.out=2*ncp)
plot(times, sapply(times, gc),type = "s", xlab = "Time", ylab = "Global covariate", 
     cex.main = 2,cex.axis = 2, cex.lab = 1.5)
dev.off()
...
for(n in sample_sizes){
  ...
  plot(l0.gam)
  save(c_o, file = paste("results_different_n/n_",n,"/c_o.RData", sep = ""))
  save(model_est, file = paste("results_different_n/n_",n,"/model_est.RData", sep = ""))
  save(c_model_est, file = paste("results_different_n/n_",n,"/c_model_est.RData", sep = ""))
  save(res_cum_bh, file = paste("results_different_n/n_",n,"/res_cum_bh.RData", sep = ""))
  save(sim_et, file = paste("results_different_n/n_",n,"/sim_et.RData", sep = ""))

  min_t_o<-max(min_bh_t)
  save(min_t_o, file = paste("results_different_n/n_",n,"/min_t_o.RData", sep = ""))
  max_t_o<-min(max_bh_t)
  save(max_t_o, file = paste("results_different_n/n_",n,"/max_t_o.RData", sep = ""))

...
for(n in sample_sizes){
  ...
  k_val<-10
  source(file ="sim_code.R")
  plot(l0.gam)

  new_subfolder<-paste("results_different_n/n_",n, sep = "")
  dir.create(new_subfolder)
  save(c_o, file = paste(new_subfolder,"/c_o.RData", sep = ""))
  save(model_est, file = paste(new_subfolder,"/model_est.RData", sep = ""))
  save(c_model_est, file = paste(new_subfolder,"/c_model_est.RData", sep = ""))
  save(res_cum_bh, file = paste(new_subfolder,"/res_cum_bh.RData", sep = ""))
  save(sim_et, file = paste(new_subfolder,"/sim_et.RData", sep = ""))

  min_t_o<-max(min_bh_t)
  save(min_t_o, file = paste(new_subfolder,"/min_t_o.RData", sep = ""))
  max_t_o<-min(max_bh_t)
  save(max_t_o, file = paste(new_subfolder,"/max_t_o.RData", sep = ""))

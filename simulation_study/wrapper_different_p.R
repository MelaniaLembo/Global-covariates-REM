...
times<-seq(0,max_t/2, length.out=2*ncp)
...
for(p in sample_sizes){
  ...
  plot(l0.gam)
  save(c_o, file = paste("results_different_p/p_",p,"/c_o.RData", sep = ""))
  save(model_est, file = paste("results_different_p/p_",p,"/model_est.RData", sep = ""))
  save(c_model_est, file = paste("results_different_p/p_",p,"/c_model_est.RData", sep = ""))
  save(res_cum_bh, file = paste("results_different_p/p_",p,"/res_cum_bh.RData", sep = ""))
  save(sim_et, file = paste("results_different_p/p_",p,"/sim_et.RData", sep = ""))

  min_t_o<-max(min_bh_t)
  save(min_t_o, file = paste("results_different_p/p_",p,"/min_t_o.RData", sep = ""))
  max_t_o<-min(max_bh_t)
  save(max_t_o, file = paste("results_different_p/p_",p,"/max_t_o.RData", sep = ""))

...
abline(v = 9,lty = 3)
abline(v = 18,lty = 3)
axis(1,c(9,18,seq(0,24,by = 6)),cex.axis = 2.5, cex.lab = 2.5)
dev.off()
...
par(mfrow=c(1,1))
pdf("final_plots/bike_data_res_cum_bh.pdf", height = 8, width = 12)
par(mar=c(5,6,4,1)+1, mgp=c(5,2,0))
plot(sort(merged_data$DateTime*60 + starting_timepoint),c_est$model$L0,type="l", xaxt="n", 
     cex.main = 2,cex.axis = 2.5, cex.lab = 2.5,  ylab = "Breslow estimator",
     xlab = "Time", col = "grey", lwd = 3)
r <- as.POSIXct(round(range(merged_data$DateTime*60 + starting_timepoint), "days"))
axis.POSIXct(1, at=seq(r[1], r[2], by="days"), format = "%d %b",cex.axis = 2.5, cex.lab = 2.5)
# calendar day on the x-axis
lines(sort(merged_data$DateTime*60 + starting_timepoint),sort(merged_data$DateTime)*c_est$coefficients,col = "black", lwd = 1.5)

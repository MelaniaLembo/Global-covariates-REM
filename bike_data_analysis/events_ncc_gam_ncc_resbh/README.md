...
- ncc_samp_resbh.R: performs nested case-control sampling on the original process. These non-events are used to compute the NCC version of the Breslow estimator for the residual cumulative baseline hazard (the _small and _big versions are equivalent but the case of shifts that area tenth and ten times the average resp.).

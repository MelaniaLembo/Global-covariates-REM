...
n<-nrow(merged_data)
ordered_shifted_et<-order(merged_data$abs_te_small)


ncc_results <- foreach(i = row_indices, .combine = 'rbind') %dopar%{
  which.event<-ordered_shifted_et[i]
  t.absevent<-merged_data$abs_te_small[which.event]
  
  # interactions that have "entered" yet and not yet "exited"
  at.risk<-setdiff(which((info$shift_small<t.absevent) & (info$exit_small>t.absevent)), 
  ...
  if(size_rs>0){ # we exclude the case in which the risk set is only comprised of only the event happening
    ...
    non.tm <- t.absevent - info$shift_small[non.event_info]

...
# dataset of all possible pairs among the randomly sampled 20 stations
load(file = "station_info.RData")
...
stations<-unique(c(info$sender,info$receiver))
...
for (i in 1:20) {
  coords_ll[i,]<- unique(info[info$receiver==stations[i],c(6,5)])
}
